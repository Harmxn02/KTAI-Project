...
We aggregate BLEU scores from our run, as found in Table~\ref{tab:baseline-comparison}, and compare them to Table 7 in \citet{hwang2021cometatomic}. Our overall scores closely match the paper's reported numbers, confirming that our evaluation pipeline successfully reproduces the baseline results.
...
Table~\ref{tab:category-level-results} breaks down the results by category. We find that the differences are rather small, with BLEU-1 scores ranging only from 0.708 to 0.763 across the three categories. Social interactions leads on BLEU-1 and BLEU-2, while event-centered relations lead on BLEU-3 and BLEU-4. This suggests that while there are some differences in performance across categories, they are not very pronounced. With the differences between categories being small, we conclude that our original hypothesis, that social interactions would outperform the other categories, is only partially supported
...
\begin{table}[h]
	\begin{tabular}{|l|c|c|c|c|}
		\hline
		\textbf{Name} & \textbf{BLEU-1} & \textbf{BLEU-2} & \textbf{BLEU-3} & \textbf{BLEU-4} \\
		\hline
		SI            & 0.763           & 0.605           & 0.498           & 0.412           \\
		PE            & 0.716           & 0.582           & 0.492           & 0.418           \\
		EC            & 0.708           & 0.598           & 0.523           & 0.462           \\
		\hline
	\end{tabular}
	\caption{BLEU scores by category}
